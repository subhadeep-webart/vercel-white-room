...
const ContactusBannerPage = () => {
  const { data, loading: isBannerContentLoading } =
    useGetContactUsPageContent("banner");
  const {
    handleUpdateContactUsPageContent,
    loading: isBannerLoading,
    refetch,
  } = useUpdateContactUsPageContent();
  ...
  if (isBannerContentLoading) {
    return (
      <AdminPageLoader content={"Loading Contact Us Page Banner Content"} />
    );
  }
  return (
