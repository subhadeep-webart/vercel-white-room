...
const BannerPage = () => {
    const { data: onTheDayContent, loading: isGetOnTheDayLoading, refetch } = useGetOnTheDayPageContent("banner");

    const { handleUpdateOnTheDayComponent, loading: isUpdatingOnTheDayComponent } = useUpdateOnTheDayComponent(refetch);
    ...
    return (
        <div className="p-3 bg-[#f0f3f8] min-h-screen">
            <div className="flex py-2 justify-between gap-2 items-center">
                <p className="text-black text-xl font-semibold my-4">On The Day Page Banner Content</p>
            </div>
            <CommonBannerForm defaultValues={onTheDayContent ?? {}} isLoading={isUpdatingOnTheDayComponent} onSubmitHandler={onBannerSubmit} />
        </div>
    )
}
