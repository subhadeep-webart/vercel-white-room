...
const BannerPage = () => {
  const {
    ...
    loading: isTestimonialContentLoading,
    refetch,
  } = useGetTestimonialPageContent("banner");

  console.log("Testimonial Content======>", testimonialContent);

  const { handleUpdateTestimonialPage, loading: isUpdatingTestimonialPage } =
    ...
    <div className="p-3 bg-[#f0f3f8] min-h-screen">
      <div className="flex py-2 justify-between gap-2 items-center">
        <p className="text-black text-xl font-semibold my-4">
          Testimonial Page Banner Content
        </p>
      </div>
      <CommonBannerForm
        defaultValues={testimonialContent}
        isLoading={isUpdatingTestimonialPage}
