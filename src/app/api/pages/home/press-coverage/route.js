...
export async function POST(request) {
  try {
    ...
    if (componentIndex > -1) {
      ...
      page.components[componentIndex].data.coverages = [
        ...coverages,
        newCoverages,
      ];
    } else {
...
export async function PUT(request) {
  try {
    ...
    const coverages = page.components[componentIndex].data.coverages || [];
    const concertIndex = coverages.findIndex(
    ...
    return NextResponse.json({
      success: true,
      coverage: coverages[concertIndex],
    });
  } catch (error) {
...
export async function DELETE(request) {
  try {
    ...
    const updatedCoverages = existingCoverages.filter(
      (coverage) => coverage._id?.toString() !== _id
    );

    if (updatedCoverages.length === existingCoverages.length) {
      return NextResponse.json(
        { error: "Coverages not found" },
        { status: 404 }
      );
    }
