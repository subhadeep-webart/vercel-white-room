...
export async function POST(request) {
    try {
        ...
        const componentIndex = page.components.findIndex(c => c.type === "media_assets");

        if (componentIndex > -1) {
            const media_assets = page.components[componentIndex].data.media_assets || [];
            page.components[componentIndex].data.media_assets = [...media_assets, newMedia];
        } else {
...
export async function PUT(request) {
    try {
        ...
        const media_assets = page.components[componentIndex].data.media_assets || [];
        const concertIndex = media_assets.findIndex(concert => concert._id?.toString() === _id);
        ...
        return NextResponse.json({ success: true, concert: media_assets[concertIndex] });
    } catch (error) {
...
export async function DELETE(request) {
    try {
        ...
        const existingAssets = page.components[componentIndex].data.media_assets || [];

        const updatingAssets = existingAssets.filter(media => media._id?.toString() !== _id);
