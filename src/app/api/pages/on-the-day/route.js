  ...
  try {
    ...
    if (!page) {
      page = new Page({
        ...
        components: [],
        title: "ON THE DAY",
        heading: "WHAT TO EXPECT FROM A NIGHT WITH THE WHITE ROOMS",
        status: "published",
      });
    }
...
export async function PUT(request) {
  try {
    ...
    const contents = page.components[componentIndex].data.contents || [];
    const concertIndex = contents.findIndex(
...
export async function DELETE(request) {
  try {
    ...
    const updatingAssets = existingAssets.filter(
      (content) => content._id?.toString() !== _id
    );
