...
import {
  forLargerShowsValidationSchema,
  getWithBookingFormValidationSchema,
...
const OptionalAddOnForm = ({ defaultValues = {}, refetch }) => {
  const {
    handleUpdateBookingPageComponent,
    loading: isUpdatingBookingPageComponent,
  ...
  const {
    ...
    defaultValues: {
      ...
      subsection3_title: defaultValues?.subsection3_title || "",
      subsection3_description: defaultValues?.subsection3_description || "",
    },
  });
  ...
  useEffect(() => {
    if (defaultValues && Object.keys(defaultValues).length > 0) {
      reset({
        ...
        section_image_url: defaultValues?.section_image_url || "",
        subsection1_title: defaultValues?.subsection1_title || "",
        subsection1_description: defaultValues?.subsection1_description || "",
        subsection2_title: defaultValues?.subsection2_title || "",
        subsection2_description: defaultValues?.subsection2_description || "",
        subsection3_title: defaultValues?.subsection3_title || "",
        subsection3_description: defaultValues?.subsection3_description || "",
  ...
  const onSubmit = async (data) => {
    try {
      const payload = { type: "optional_add_on", data: data };
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="w-full relative">
            ...
            {errors.subsection1_description && (
              <FormErrorText
                errorText={errors.subsection1_description.message}
              />
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="w-full relative">
            ...
            {errors.subsection2_description && (
              <FormErrorText
                errorText={errors.subsection2_description.message}
              />
            )}
          </div>
        </div>
      ...
      <div className="w-full bg-white shadow-2xl space-y-4 my-2 font-semibold text-lg px-8 py-4 rounded-2xl">
        <p className="text-md font-semibold text-black  my-4">Subsection 3</p>
        <div className="grid grid-cols-1 gap-6">
          <div className="w-full relative">
            <Label htmlFor="subsection3_title" className="mb-1 block">
              Title
            </Label>
            <Input
              id="subsection3_title"
              placeholder="Add Subsection2 Title"
              {...register("subsection3_title")}
            />
            {errors.subsection3_title && (
              <FormErrorText errorText={errors.subsection3_title.message} />
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="w-full relative">
            <Label htmlFor="subsection3_description" className="mb-1 block">
              Description
            </Label>
            <Textarea
              row={4}
              id="subsection3_description"
              placeholder="Add Concert Name"
              {...register("subsection3_description")}
            />
            {errors.subsection3_description && (
              <FormErrorText
                errorText={errors.subsection3_description.message}
              />
            )}
          </div>
        </div>
...
export default OptionalAddOnForm;
