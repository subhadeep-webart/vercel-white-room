...
const AddPressCoverageForm = ({ isEdited = false, defaultValues = {} }) => {
    ...
    const { handleCreatePressCoverage, loading: isCreatingPressCoverage } = useCreatePressCoverage();
    const { handleUpdateCoverage, loading: isUpdatingCoverage } = useUpdateCoverage();
    const [songUrl, setSongUrl] = useState();
    const [imageUrl, setImageUrl] = useState(
        defaultValues?.poster_image || ""
    );
    const [songImageUrl, setSongImageUrl] = useState(defaultValues?.song_image || "");
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="w-full relative z-0">
                    ...
                    {typeof window !== undefined && (
                        <CldUploadWidget
                            ...
                            onSuccess={(result) => handleUploadSuccess(result, "image", "poster")}
                        >
                            {({ open }) => (
                                <Button
                                    onClick={() => open()}
                ...
                <div className="w-full relative z-0">
                    ...
                    {typeof window !== undefined && (
                        <CldUploadWidget
                            ...
                            onSuccess={(result) => handleUploadSuccess(result, "image", "song-image")}
                        >
                            {({ open }) => (
                                <Button
                                    onClick={() => open()}
            ...
            <div className="flex justify-end">
                ...
                >
                    {isCreatingPressCoverage || isUpdatingCoverage ? <Loader /> : "Submit"}
                </Button>
            </div>
        </form>
    );
