...
const ArtistNamesComponent = ({
  ...
  const { handleDeleteArtist, loading: isDeleteLoading } =
    useDeleteArtist(refetch);
  return (
    <div className="bg-white shadow-lg px-4 py-2 rounded-md mt-2.5">
      <div className="w-full flex justify-between items-center">
        <p className="text-black py-2 text-lg font-semibold px-2">
          {headerName}
        </p>
        <Button
      ...
      <div className="flex flex-wrap justify-start gap-2">
        {artists?.map((artist) => (
          <ArtistNameDisplayComponent
            ...
            artist={artist}
            handleDeleteArtist={() =>
              handleDeleteArtist({ section, _id: artist?._id })
            }
          />
        ))}
      </div>
