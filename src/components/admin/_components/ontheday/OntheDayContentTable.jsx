...
const OntheDayContentTable = ({ columns, data, refetch }) => {
  const { handleDeleteOnTheDayPageContent, loading: isDeletingContent } =
    useDeleteOntheDayPageContentById(refetch);
  return (
                    ...
                    >
                      {col.accessorKey === "actions" ? (
                        <div className="flex justify-end gap-2">
                          ...
                          <AlertBox
                            onDelete={handleDeleteOnTheDayPageContent}
                            rowData={row}
                          />
                        </div>
                      ) : col.accessorKey === "section_image_url" &&
                        row[col.accessorKey] ? (
                        <SmallImageCard imageUrl={row[col.accessorKey]} />
                      ) : row[col.accessorKey] ? (
                        <p
                          dangerouslySetInnerHTML={{
                            __html: row[col.accessorKey],
                          }}
                          className="text-left"
                        ></p>
                      ) : (
