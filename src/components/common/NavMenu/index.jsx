...
const NavMenu = ({ isOpen, toggleNavBar }) => {
  ...
  const handleNavMenuMouseEnter = (item) => {
    ...
    gsap.to(`[data-key="${item.key}"] a`, {
      color: "#ffffff",
      duration: 0.5,
      scale: 1,
    });
    gsap.to(`.menu-link-item-holder:not([data-key="${item.key}"]) a`, {
      scale: 0.98,
      color: "#8f96a5",
      duration: 0.5,
    });
  ...
  const handleImageFade = (img) => {
    gsap.fromTo(
      ...
      { opacity: 0 },
      {
        opacity: 1,
        duration: 0.5,
        onComplete: () => setCurrentImageSrc(hoverImageSrc),
      }
    );
  };
  ...
  return (
    <div
      className={styles.nav_menu_background}
      id="menu-link-holder"
      ref={containerRef}
    >
      <div className="w-full h-full flex gap-10 px-20 py-14 mt-5">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-between">
          ...
          <Link
            href={INSTAGRAM_LINK}
            target="_blank"
            className="text-white font-bold text-md flex items-center gap-2"
            onClick={toggleNavBar}
          >
